...
export default function ReservationDetailPage() {
  ...
  return (
    <div className="max-w-3xl">
      <button onClick={() => router.back()} className="text-sm text-stone-500 hover:underline mb-4">
        ← Back
      </button>

      <div className="flex items-start justify-between flex-wrap gap-3 mb-6">
        <div>
          <h1 className="text-xl font-semibold text-stone-900">{reservation.guest.name}</h1>
          <p className="text-sm text-stone-500 mt-0.5">
            {reservation.checkIn} → {reservation.checkOut} · {reservation.rooms.map((r) => r.room.roomNumber).join(', ')}
          </p>
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            <span className="text-xs font-medium px-2 py-1 rounded-full bg-stone-100 text-stone-700 capitalize">{reservation.status.replace('_', ' ')}</span>
            <span className="text-xs font-medium px-2 py-1 rounded-full bg-stone-100 text-stone-700 capitalize">{reservation.source.replace('_', '.')}</span>
        ...
        {canAct && (
          <div className="flex gap-2 flex-wrap">
            {reservation.status === 'confirmed' && (
              <button onClick={() => checkIn()} disabled={busy} className="rounded-md bg-emerald-800 text-white text-sm font-medium px-3 py-2 hover:bg-emerald-900 disabled:opacity-60">
                Check in
              </button>
            )}
            {reservation.status === 'checked_in' && (
              <button onClick={checkOut} disabled={busy} className="rounded-md bg-emerald-800 text-white text-sm font-medium px-3 py-2 hover:bg-emerald-900 disabled:opacity-60">
                Check out
              </button>
            )}
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div className="bg-white border border-stone-200 rounded-xl p-4">
          <h2 className="text-sm font-medium text-stone-700 mb-3">Guest</h2>
        ...
        <div className="bg-white border border-stone-200 rounded-xl p-4">
          <h2 className="text-sm font-medium text-stone-700 mb-3">Balance</h2>
          <p className="text-lg font-semibold text-stone-900">£{balance.toFixed(2)}</p>
          <p className="text-xs text-stone-400">Invoiced minus payments received</p>
        </div>

        <div className="bg-white border border-stone-200 rounded-xl p-4 sm:col-span-2">
          <h2 className="text-sm font-medium text-stone-700 mb-3">Status history</h2>
          <ul className="text-sm text-stone-600 space-y-1">
            {reservation.statusHistory.map((h, i) => (
              <li key={i}>
                ...
                {h.note && <span className="block text-xs text-stone-400">{h.note}</span>}
              </li>
            ))}
          </ul>
        </div>
        ...
        {!reservation.paymentViaOta && canAct && (
          <div className="bg-white border border-stone-200 rounded-xl p-4 sm:col-span-2">
            <h2 className="text-sm font-medium text-stone-700 mb-3">Payments</h2>
            ...
            <ul className="text-sm text-stone-600 mt-3 space-y-1">
              ...
              {payments?.length === 0 && <li className="text-stone-400">No payments yet.</li>}
            </ul>
          </div>
        )}

        <div className="bg-white border border-stone-200 rounded-xl p-4 sm:col-span-2">
          <h2 className="text-sm font-medium text-stone-700 mb-3">Invoices</h2>
          {invoices?.length === 0 && <p className="text-sm text-stone-400">Generated automatically at checkout.</p>}
          {invoices?.map((inv) => (
            <div key={inv.id} className="border-t border-stone-100 pt-2 mt-2 first:border-t-0 first:pt-0 first:mt-0">
              ...
              <ul className="text-xs text-stone-500 mt-1">
  ...
  return (
    <form onSubmit={submit} className="flex items-end gap-2 flex-wrap">
      ...
      <div>
        ...
        <input type="number" min={0.01} step="0.01" value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="rounded-md border border-stone-300 px-3 py-1.5 text-sm w-28" />
      </div>
      <div>
        <label className="block text-xs text-stone-500 mb-1">Method</label>
        <select value={method} onChange={(e) => setMethod(e.target.value)} className="rounded-md border border-stone-300 px-3 py-1.5 text-sm">
          <option value="card">Card</option>
          <option value="cash">Cash</option>
          <option value="bank_transfer">Bank transfer</option>
        </select>
      </div>
      <button disabled={busy || amount <= 0} className="rounded-md bg-emerald-800 text-white text-sm font-medium px-3 py-1.5 hover:bg-emerald-900 disabled:opacity-60">
